...
{
  int n = leftDeck->size;
  int m = rightDeck->size;
  memcpy(leftDeck->cards, origDeck.cards, n * sizeof(origDeck.cards[0]));
  memcpy(rightDeck->cards, &origDeck.cards[n], m * sizeof(origDeck.cards[0]));
}
#endif
...
void interleave(CardDeck leftDeck, CardDeck rightDeck)
{
  ...
  int maxSkip = MAX(leftDeck.size, rightDeck.size);
  for (int skip = 0; skip < maxSkip; skip++)
  {
    int i = 0;
    int j = 0;
    while (i < leftDeck.size || j < rightDeck.size)
    {
      bool f = skip & 1;
      if (i >= leftDeck.size)
      {
      ...
      if (i < leftDeck.size && f)
      {
        printf("%c ", leftDeck.cards[i++]);
        continue;
      }
      if (j < rightDeck.size && f)
      {
        printf("%c ", rightDeck.cards[j++]);
        continue;
      }
    }
    printf("\n");
...
{
  int psize = origDeck.size;
  //(memcpy(origDeck.cards, deck, MAX_SIZE * sizeof(origDeck.cards[0])))
  for (int n = 1; n < psize; n++)
  {
    int m = psize - n;
    CardDeck *left = malloc(sizeof(CardDeck));
    CardDeck *right = malloc(sizeof(CardDeck));
    left->size = n;
    right->size = m;
